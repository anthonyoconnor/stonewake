...
Full accessible accommodation blocks every new living arrival, including newly unlocked types. **Dormitory is full** raises an icon beside the sidebar, with an expansion explanation and **Build Dormitory** action. Closing its details keeps the icon; dismissing removes it for that episode and retains it in history. Freeing space clears the condition, and filling again raises a new icon. First natural recruitment of each type in an area also announces the arrival and offers to locate the new resident. See [notification controls](gameplay-interface.md#messages-and-the-question-mark-button). Existing residents are never removed to meet recruitment targets. Lost den support still uses ordinary need grace/departure rules.
...
Reinforcement is the exception to the productive assignment window: a worker finishes its current reinforcement tile, releases that job reservation normally, and re-evaluates the full work pool before starting another wall. Newly reachable excavation, claiming, resource work, hauling and explicit construction therefore take precedence over another reinforcement tile even if the old assignment has time remaining. Already staffed targets are reserved; spare workers may reinforce if all higher-priority targets are staffed, unreachable or unsafe. This tile boundary avoids repeated switching and discarded progress. Threat escape still interrupts immediately; after safety returns, the old reinforcement assignment cannot override useful terrain work.
