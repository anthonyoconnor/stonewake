# M35–M42 level overhaul specifications (historical)

This is the original authorized roadmap, preserved with its initial targets and dependencies. Read the [current guide](../../level-overhaul.md) for authoring rules, the [active notes](../../development-plan.md) for remaining work, and [development history](../development-history.md) for actual completion evidence. Statements below describing pending implementation are historical.

Active milestone specifications for redoing all five campaign levels and the seven additional standalone Free Play levels. The [development plan](../../development-plan.md) owns progress; this document owns redesign criteria, provisional size targets and milestone scope. The current playable maps remain documented in [Levels](../../levels.md) until their replacements are implemented.
