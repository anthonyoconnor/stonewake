...
The active roadmap implementation now adds a five-area campaign with gradual room, role, defense and spell unlocks; living regional habitats; reclaimable dwarven ruins; authored resource routes; procedural audio; refined animation; and ordinary underground source/pointer lighting. Campaign travels from Border Foothold through Fungal Hollows, Fallen City and Crystal Divide to Royal Deep. Every area has a concealed onward Hearthstone that residents must physically reach and activate. See [Levels](levels.md) for the progression brief and [development plan](development-plan.md) for verification status and remaining review. Guard Posts, specialist retreat and door repairs/upgrades remain deferred.

Launch opens the approved **Campaign / Free Play / Settings** menu. Free Play lists the five authored campaign areas with their arrival plans and seven deliberately retained prototype levels with the full implemented catalog. Each begins independently with a fresh crew; research still requires a Library. The sidebar ☰ returns to the menu after confirmation. Session settings include animation preference, edge scrolling and master/music/effects volume with mute. The sidebar ♪ button adjusts sound during play. Reloading resets everything. The themed loading screen covers startup and world transitions; failed transitions offer Retry/Reload.
...
Project-wide development instructions are in [AGENTS.md](project-instructions.md).
...
**Debug → Level preview** opens a whole-level map with fog disabled, enemy positions, encounter sources and both Hearthstones. Choose the current world or any Campaign/Free Play starting layout, then click a position or select an enemy for details. Play pauses while the preview is open; closing it preserves the game's exploration and prior pause state.

**Load full level** in that modal opens the selection as a playable 3D test world with fog off and all enemies visible. The current-world option uses a copy; named levels start with their normal crew and economy. It starts paused and frames the whole level. Pan, orbit and zoom normally, use Resume simulation to play, or Return to stronghold to restore your retained game.
...
Choose **Spells, traps & Hearthstones** in the same group for the [arcana comparison room](arcana-overhaul.md): thirteen starting/refined pairs for all nine spells, both traps and both Hearthstone forms. Select an exhibit, preview its states, play/pause/step, turn and zoom, or expand **Concept art** beside its live model. Twelve new individual sheets and their generation prompts are retained in the concept folders. **Back to comparison** restores the controls after opening another panel without reloading the room; **Return to stronghold** restores your game. All three studios also remain in the grouped **Debug → Test harnesses → Additional test scenarios** selector as `graphics-gallery`, `terrain-comparison` and `arcana-gallery`.
...
Click the compact **Hearth** button beneath the gold/population totals for core health and the onward objective. After discovery, request activation: an available dwarf must walk beside the stone and secure it for eight uninterrupted seconds. Nearby enemies or urgent needs interrupt work; the request retries automatically. Success and defeat freeze the area, show a sidebar result and offer **Restart area**. The starting base and treasury stay fixed within each area. In campaign play, each victory offers travel to the next authored area; the Royal Deep victory offers **Begin a new journey**. Travel retains researched spell knowledge and building unlocks, while residents, gold, buildings, queues and prepared spell charges reset. Arrival at each area introduces its next room, role or tool; Royal Deep introduces stone bridges. **Restart area** restores that area's arrival state, including its carried knowledge.
...
|---|---|
| Level dimensions, openings and resource seams | [Authored campaign maps](../../src/content/campaign-levels.ts) and [retained prototype definitions](../../src/content/levels.ts) |
...
| Sound cues, biome score and session volumes | [Audio design](../../audio-design.md), [cue definitions](../../src/content/audio.ts), [event adapter](../../src/game/audio-events.ts), [audio service](../../src/view/audio.ts) |
...
Keep agreed rules distinct from experiments and proposed numbers. The authored campaign, crossings, repeat Library use and automatic trap reset are implemented and tested. Economy, combat and research pacing remain tunable prototype values; crowd intersections and procedural art have documented limits. Audio listening feedback is pending. Repairs, assigned guard duties and general specialist retreat remain deferred. Current status is recorded in [development-plan.md](development-plan.md).
...
**M24 combat balance and test room are implemented.** Open Debug → Test harnesses → Combat test room for repeatable hound, specialist and regional enemy matchups, support options, reset, pause and return. See [measured roles and results](characters.md#combat-balance-and-test-room-m24). M25/M25.1 menus, free play and loading screens are implemented.

**Debug → Test harnesses → M33 lighting test room** provides a paused furnished settlement, lava/water crossings and a sealed fog pocket. Adjust ambient/source light, glow and pointer illumination, choose a camera view, compare the current renderer, reset, or return to your retained stronghold. During development, `?scenario=lighting&paused=1` opens it directly. The [lighting comparison harness](../graphics-pass.md#m33-lighting-test-room) remains available alongside the ordinary biome/source/pointer lighting now used in Campaign and Free Play.
