...
The **Training & research** group tunes training duration, interval, level cap and work-speed bonus, shared research pacing. Spell strength, duration and other effect values are in the **Spells** group and apply to future casts. Specialist arrival cadence is under **Economy & world**. Spell research/preparation times and casting prices come from the spell registry and appear in the same group. Training/research progress is retained during tuning and room changes. Normal arrivals require explicit world enablement and are off by default in room layouts; **Test automatic specialist arrivals** enables the same rules there. These systems live in `src/game/progression.ts`, `research.ts` and `recruitment.ts`; work scheduling remains in `simulation.ts`.
