...
Status: **M1–M9 complete, including M5.1; M10–M19 planned.** TypeScript and Babylon.js are confirmed. The user authorized implementation on 2026-09-07 and subsequently requested a graphics and animation pass as M9, which was added after M8 was complete. The subsequent roadmap request adds M10–M19 for the remaining design and establishes finding a new Hearthstone as each level's onward objective. These new milestones are planning scope, not completed features or an instruction to begin implementation. Verification and prototype limitations are recorded below.
...
**Current character model:** dwarfs start at level 1 and advance through five explicit per-type rows. Only active Training Room practice grants levels; health, damage, attack interval and work multiplier come from the reached row. See [Character levels](characters.md#character-levels-and-training) for the current table and requirements, which supersede older flat 12-second training and +8% upgrade records below.
