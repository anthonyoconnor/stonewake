...
**Room presentation:** the user-authorized [room overhaul](room-overhaul.md) adds six individual concepts, distinctive per-room floor textures and sparse equipment sets. A second visual pass adds weathered materials, footprint-aware perimeter bands, single Kitchen/Library centerpieces, sculpted bedding, substantial joinery and inward-facing equipment with integrated lamps. Treasure Rooms show sequential tile-sized piles from their actual stored wealth; Dormitories show only assigned residents' role-specific bedding. Kitchen and Workshop use coherent small equipment sets, Training has upright targets, and Library uses long shelves. Original room rendering/layout remains in the terrain reference studio. Floor-area prices, capacity and services are unchanged.
...
**Debug navigation:** **Level preview** shows the current world or any playable starting layout as a whole-level map with fog disabled, living enemy positions, encounter sources and Hearthstones. Map inspection is read-only and pauses simulation while open. **Load full level** opens an independent playable 3D test world with fog off, all enemies visible and the whole level framed; it starts paused and retains the stronghold for return. **Before & after** directly opens **Characters**, **Terrain & rooms**, and **Spells, traps & Hearthstones**. **Back to comparison** restores the current studio controls without reloading or changing its camera and preview. **Test harnesses** retains gameplay launchers and additional scenarios grouped by purpose; display rooms omit simulation advancement, resident diagnostics and test-actor setup. Shared session settings remain distinct from current-world tools.

**Spells, traps and Hearthstones:** all nine current spell effects, both traps and both Hearthstones now have concept-driven models and a permanent `arcana-gallery` with thirteen paired starting/refined exhibits, state playback and in-panel concept references. Twelve missing concept sheets were generated; the approved main Hearth and Stonehand character concepts remain authoritative. The starting renderers and their material policy are retained separately. See [the arcana overhaul record](arcana-overhaul.md) for per-item references, inspection controls and practical limits. Gameplay rules and prices are unchanged; successful Stonehand creation now emits a short assembly effect at its arrival square.
