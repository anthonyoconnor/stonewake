...
- **Left controls:** Carved Stronghold artwork with dedicated basalt/bronze skins, layered chamfered sockets and embossed SVG pictograms. Controls share an aligned inner column clear of the carved rails and bottom braces; a separate scroll lane preserves alignment when details expand, and paired utility rows use equal-width cells. Icon-only navigation, secondary disclosures, blue selection outlines/runes and pause remain available. Redundant captions, slogans, empty threat reports and default tool cards are removed. [Interface text budget](gameplay-interface.md#sidebar-text-budget) records why the remaining text is visible.
