...
All ten creatures in the [enemy concept gallery](concept-art/enemies/README.md) are implemented as editable definitions in [src/content/enemies.ts](src/content/enemies.ts). The values below establish distinct tactical roles and remain tunable. The authored campaign has integrated route verification; historical results live in the archive.
...
Hounds provide early defense; trained Warriors, manufactured traps and Library spells support later armored and volcanic groups. Use the [combat test room](characters.md#combat-balance-and-test-room) for repeatable matchups. Historical results are archived and do not guarantee outcomes for arbitrary group sizes or layouts.
