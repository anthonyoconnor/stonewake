...
The sidebar follows the approved [Carved Stronghold concept](concept-art/sidebar/README.md): a purpose-made chipped basalt/bronze surround, nine-sliced chamfered sockets, broad ivory/bronze SVG pictograms and blue selected outlines with a corner rune. The [production skins and exact prompts](public/art/sidebar/README.md) are separate from the concept board. The title, framed resource counters, blue Hearth strip and paired utility rows follow the concept's composition while retaining the text budget below. Borders and decoration must not compete with the icons or reduce useful viewing space. Text and numbers are permitted in the sidebar, but long explanations should appear only when requested or as messages.

The frame has a reserved, non-interactive margin: its carved rails occupy the outer 11% on each side, with additional clearance for control borders and focus outlines. The map, resource row, Hearth strip, category buttons, action grids, selected header, disclosures and utilities share one inner column. The action panel's separate scrollbar lane must not shift that column when details expand. Bottom controls sit above the corner braces, and cancelling a tool must not resize its neighboring buttons. Browser checks enforce frame clearance and alignment in resting, selected, expanded and compact layouts.

## Sidebar text budget

Visible text must identify the current choice, quantify a consequence, explain an actionable problem, or name an explicit action. The resting Rooms view contains only the wordmark, gold/population totals and Hearth health; the wordmark retains the approved concept's identity. Icons, selected outlines/runes and dimming carry the navigation and availability states. Tooltips remain available by pointer and keyboard focus.

| Text retained | Why it earns space | When shown |
|---|---|---|
| Gold, population and Hearth health | Determines affordability, workforce size and core danger | Persistent, paired with icons |
| Selected action name, cost and stock | Confirms what a click will purchase/place and whether stock exists | A relevant room, construction tool, spell or defense is selected |
| Placement tile count, total cost and added capacity | Shows the consequence of committing the current footprint | While targeting; errors explain blocked actions |
| Research state/progress and Research/Resume/Pause | Distinguishes preparation from casting and lets the player manage it | Selected spell |
| Inspected identity, health, activity, effects and room usage/access | Explains the selected resident/room/fixture and its current condition | Explicit inspection; the resident list and effect descriptions expand on demand |
| Door modes, bolt facing and dismantle refund | Defines changes to passage, firing direction or removal | Relevant defense inspection/selection |
| Activation, outcome and travel/restart buttons | Provides the next required step, including irreversible run changes | Hearth details or an ended area |
| Notification details and action results | Explains a changed situation, failed action or completed purchase | An opened notification, placement feedback or action result |
| Details, Effect and other secondary action labels | Names what a disclosure/action will reveal or do | Inside the selected item or expanded management panel |

Removed: the brand tagline, map caption, grid dimensions, map-name row, resource/category labels, footer slogan, default excavation card, duplicated active-tool name, workforce heading/total/instruction, default defense introduction, separate Inspect button, Known threats heading, empty threat/fixture reports, decorative-furniture reminder, and repeated excavation/selling instructions. Level identity remains in map hover help and Hearth details. Room capacity per tile lives in selected-header hover/focus help; actual footprint and inspection totals stay visible. Door-mode explanations use button tooltips. Workshop, research, resident management and discovered-source disclosures use icons; source reports only appear when something is discovered. Debug and Help remain explicit opt-in text surfaces.
...
The preview modal's **Load full level** action opens the selected map in the actual 3D game, fully revealed with every living enemy rendered regardless of resident sight range. It uses an independent copy of the current world or a normal standalone start for a named level, retaining the stronghold through the existing test-world workflow. It starts paused with the camera framing the entire level. Normal camera and gameplay controls remain available; Resume simulation runs the revealed test world, and Return to stronghold restores the retained game and its pause state. Revealing terrain can trigger ordinary discovery/encounter behavior when this test world resumes. Enemy display visibility does not grant combat or spell targeting outside normal sight. Replacing or leaving the test world clears the enemy display override.
