...
- The current world tool is always named above the lower controls. Its ×, Escape or right-click returns to excavation. Releasing a drag over the sidebar cancels it. Focused buttons, disclosures, inputs and dialogs do not drive the world camera; pointer movement at sidebar edges currently does not pan it. The planned M34 fix supersedes that behavior at the outer left viewport edge, which must pan left even over the sidebar; ordinary sidebar interaction remains stationary. Closing the full map with Escape preserves the tool that was active before the map opened.
...
## Planned lighting and edge-panning changes
...
[M34](development-plan.md#m34--left-edge-camera-panning-across-the-sidebar) addresses the user-reported failure to pan left at the far-left edge. The outer game viewport edge must pan even over the sidebar; its internal boundary with the world is not an edge-pan trigger. Ordinary panel interaction and intentional menu/modal camera suppression remain unchanged. Both changes are planned, not yet implemented.
...
Debug separates current-world actions from Test harnesses. Shared configuration and free construction explicitly affect both worlds. Harnesses start paused, replace the previous test world, and retain the stronghold in memory. Every test panel exposes Pause/Resume and Return to stronghold; returning restores the previous stronghold pause state. Restart stronghold is only shown in the ordinary game. Needs setup buttons describe their effect and required rooms.
