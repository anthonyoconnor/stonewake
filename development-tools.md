...
### Terrain performance comparisons

`npm run verify -- lighting-lab --browser=terrain-batches` compares original and batched rendering of the lighting chamber, Royal arrival, fully revealed Royal overview and permanent terrain comparison. It checks framebuffer differences, accelerated/original tile picks, light membership, camera/pointer changes, discovery, lighting toggles and world cleanup. Captures and results go to ignored `test-results/terrain-batches/` (override with `VISUAL_FOLDER`).

For matched live measurements, run `node scripts/terrain-batches-browser.mjs --profile-world=<developed-world.json>` against the same paid fixture used above. The script restores identical simulation state for each window, warms for ninety paused frames, then measures ten seconds of normal simulation. Batching and accelerated picking run off/on/on/off for both a stationary camera and an orbiting camera with the pointer light active. A browser-only module hook restores the original picking function in control windows. Reports include frame mean/p95, simulation advancement, active-mesh cost, hardware and the fixture hash. This isolates this optimization pass on the current renderer; it is not a comparison of different maps or a new gameplay acceptance run. Keep other browser/CPU workloads idle and source unchanged during timing.
