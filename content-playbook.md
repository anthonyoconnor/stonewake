...
Current rooms use sparse visual plans and floor palettes in `src/content/room-visuals.ts`, and the current model renderer in `src/view/room-furnishing-models.ts`. Registered extensions without a visual plan retain the original furnishing fallback described below. Gold piles and resident bedding are derived presentation in `src/game/room-decoration.ts`; do not use their meshes as service capacity or navigation obstacles. The pre-overhaul renderer and layout remain preserved for comparison. See [room overhaul](room-overhaul.md).
...
4. Start the game. The room joins the generated Room Layout Studio catalog and configuration tab. Normal room choices also require the arrival's content availability: add approved unlocks in `src/content/campaign.ts` and the relevant `src/content/playable-levels.ts` setup. Select it in the studio and load example shapes; no separate sidebar markup registration is required.
...
7. Add a focused service regression and browser playtest. Update the relevant design, development plan and this playbook if a new shared service was introduced. Use `npm run verify -- rooms --browser=rooms` or a narrower relevant scope for ordinary room changes; reserve full suites and production checks for major integration. Review `git diff --check`, commit the completed chunk, and keep the local server available when using browser checks.
...
| `training` | Concurrent eligible trainees; one gained level ends the visit, releases its slot and starts a personal cooldown. Partial progress belongs to the resident. |
...
1. Add one `CharacterDefinition` to `src/content/characters.ts`: unique ID, name, nonempty names list, color, speed multiplier, appearance, capabilities, attraction services and an explicit `levels` table. Each level row supplies `level`, `trainingSeconds`, `health`, `damage`, `attackSeconds`, `workMultiplier` and `wage`. Use consecutive levels starting at 1, with zero training seconds for the spawn level; later rows specify active practice required to enter them. Specialists have five levels; Stonehands, Cave Hounds and the retained Miner have one. `helmet`, `braids`, `warrior`, `runesmith`, `stonehand` and `hound` are reusable models; any can be used by any type. Do not branch gameplay on the type's name or appearance.
2. Select existing capabilities: `mine`, `haul`, `claim`, `reinforce`, `buildWall`, `research`, or a capability named by a crafting recipe. `fight` permits autonomous pursuit and rally response; `defend` permits attacks only on enemies already within melee reach. Engineers, Runesmiths and the legacy Miner use `defend`; Stonehands have no attack capability; a damage value or a higher level alone must not grant pursuit or rally. Movement is shared. `construct: true` skips living support/needs, wages and morale; `animal: true` uses den food/rest and skips wages. Level-table length determines whether a type can gain levels.
...
7. Update `characters.md`, record completed checks in the archive, run focused simulation checks with one source/test typecheck and a relevant browser check when presentation changes, then commit.
...
There is no general public mod loader or editor. Room service capacity comes from floor area and the tunable per-tile value; cosmetic objects have no gameplay role. New content still needs balance, real-access and layout checks. Door/trap placement and autonomous combat face authored encounters/raids and debug raiders. Normal worker creation uses Stonehands; legacy Miner purchases remain in fixtures. Physical wages, Hearth damage, departure and five-area campaign progression are implemented. Guard duty and maintenance are deferred. Check the current inventory for verification status of additions.
