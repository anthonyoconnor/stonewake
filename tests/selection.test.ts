...
test('default excavation toggles once per gesture and cancelling construction restores it',()=>{
 ...
 const canvas=Object.assign(new EventTarget(),{getBoundingClientRect:()=>({left:0,top:0}),setPointerCapture(){}});
 ...
 try{
  assert.equal(selection.tool,'dig');click();assert.equal(tileAt(world,10,10)!.designated,true);click();assert.equal(tileAt(world,10,10)!.designated,false);
  ...
  selection.setTool('treasure');click(7,7);assert.equal(tileAt(world,7,7)!.room,'treasure');
  ...
  click(7,7);assert.deepEqual(selection.selected,{x:7,z:7});
