...
test('an additive food-room definition reuses tile capacity, appearance, meals and reclaim services',()=>{
 ...
 try{
  ...
  const stats=roomStats(w,layout[0]);assert.equal(stats.usable.filter(f=>f.service==='dining').length,36);assert(stats.facilities.some(f=>f.model==='stove'));
  addResidents(w,'miner');w.agents[0].hunger=.1;w.furnishings=[];
  for(let i=0;i<1200&&!w.agents[0].meals;i++)tick(w,.05);assert.equal(w.agents[0].meals,1);
  assert.equal(actionIconSvg(copy.id),actionIconSvg('kitchen'));assert.equal(roomLook(copy.id).floor,roomLook('kitchen').floor);
  reclaimRoom(w,layout);assert.equal(w.roomServices.filter(s=>s.room===copy.id).length,0);assert.equal(w.spent,0);
...
test('an additive dwarf and recipe use existing movement, needs, attraction and staffed crafting',()=>{
 ...
 try{
  ...
  assert.equal(attractionStatus(w,dwarf.id),'Room and settlement support are available.');
