...
test('camera inputs preserve pan orientation and orbit around a fixed target',()=>{
  ...
  const emit=(target:EventTarget,type:string,values:object={})=>target.dispatchEvent(Object.assign(new Event(type,{cancelable:true}),values));
  ...
  try{
    ...
    for(const [x,y,dx,dz] of [[1,400,-1,0],[1199,400,1,0],[600,1,0,1],[600,799,0,-1]]){
      const before=pose();emit(win,'pointermove',{clientX:x,clientY:y,pointerType:'mouse'});controls.update(.1);const after=pose();
      near(after.alpha,before.alpha);near(after.beta,before.beta);near(after.radius,before.radius);
  ...
  }finally{scene.dispose();engine.dispose();Reflect.deleteProperty(globalThis,'window');Reflect.deleteProperty(globalThis,'document');}
});
