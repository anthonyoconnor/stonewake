...
test('removing the last treasury preserves its gold and gives immediate sale credit',()=>{
 const w=createRoomLab();buildRoom(w,'treasure',plot);const stores=w.roomServices.filter(f=>f.room==='treasure');stores[0].stored=50;stores[1].stored=20;const before=goldTotal(w);
 reclaimRoom(w,plot);assert.equal(w.roomServices.filter(f=>f.room==='treasure').length,0);assert.equal(w.tiles.reduce((n,t)=>n+t.loose,0),70);assert.equal(goldTotal(w),before-70+150);assert(w.roomServices.some(f=>f.id==='hearth-treasury'));
});
test('Kitchen reclaim removes resident support and rebuilding restores assignments',()=>{
 const w=createRoomLab();buildRoom(w,'kitchen',plot);addMiners(w,3);
 assert.equal(w.roomServices.filter(f=>f.service==='dining'&&f.assigned!==undefined).length,3);
 reclaimRoom(w,plot);tick(w,.05);assert.equal(w.roomServices.filter(f=>f.service==='dining').length,0);
 buildRoom(w,'kitchen',plot);tick(w,.05);
 assert.equal(w.roomServices.filter(f=>f.service==='dining'&&f.assigned!==undefined).length,3);
});
