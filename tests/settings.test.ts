...
test('settings validate atomically and drive new-world economy and live mining',()=>{
 const original=settingValues();try{
  ...
  const w=createWorld(prototypeLevel);assert.equal(w.allowance,17);assert.equal(w.roomServices[0].capacity,180);
  for(const t of w.tiles){t.known=true;t.terrain='bedrock';t.core=false;}
  Object.assign(tileAt(w,5,5)!,{terrain:'floor',claimed:true});const earth=tileAt(w,6,5)!;earth.terrain='dirt';w.furnishings=[];w.roomServices=[];addMiners(w,1);designate(w,[earth]);
