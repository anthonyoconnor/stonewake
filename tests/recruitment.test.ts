...
test('specialists arrive through shared eligibility only when enabled and stop at room or bed capacity',()=>{
 ...
 try{
  ...
  run(w,20);const population=w.agents.length;assert(population<=w.furnishings.filter(f=>f.service==='rest').length);
...
test('painted rooms, missing food, occupied specialist capacity and an inaccessible Hearth do not qualify arrivals',()=>{
 const w=settlement();
 for(const f of w.furnishings)if(f.service==='cooking')f.stored=0;
 assert.match(recruitmentStatus(w,'warrior').message,/food/);
 for(let i=0;i<100;i++)produceFood(w,1);
 ...
 const painted=createRoomLab();buildRoom(painted,'training',[{x:8,z:8}]);assert.match(recruitmentStatus(painted,'warrior').message,/training capacity/);
});
