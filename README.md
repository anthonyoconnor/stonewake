...
Open **Debug → Room layouts** to build and inspect implemented rooms with the normal grid tools. Example footprints and the test dwarf catalog make services easy to exercise. **Load visual showcase** creates example rooms with test gold, residents, queued crafting jobs and spell research. Click Rooms to return to the studio controls after inspecting Dwarfs; Return to stronghold restores the game you left in memory.
