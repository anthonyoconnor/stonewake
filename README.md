...
The [graphics overhaul](graphics-overhaul.md) includes permanent comparison studios under **Debug → Test harnesses → Additional test scenarios**. Load `graphics-gallery` for all sixteen starting/refined character pairs, with synchronized front/back/turn and zoom controls. Load `terrain-comparison` for matching original/refined terrain, six furnished rooms, hazards and bridges across all five regional palettes. Original renderers are retained in source. The [new stronghold concept](concept-art/terrain/stronghold-cohesion-v2.png) and [generated surface assets](public/art/terrain/README.md) record the visual target and material provenance.
...
The [spell, trap and Hearthstone overhaul](arcana-overhaul.md) adds `arcana-gallery` in the same test-scenario selector: thirteen starting/refined pairs for all nine spells, both traps and both Hearthstone forms. Select an exhibit, preview its states, play/pause/step, turn and zoom, or expand **Concept art** beside its live model. Twelve new individual sheets and their generation prompts are retained in the concept folders. The original renderers and materials remain archived; Return to stronghold restores your game.
