...
import { characterLevel, earnsWages } from '../content/characters.ts';
...
export function tickPayday(w: World) {
  ...
  while (w.elapsed + 1e-8 >= w.nextPaydayAt) {
    for (const a of w.agents.filter(a => alive(a) && earnsWages(a.type))) {
...
export function payrollStatus(w: World) {
  const statuses = w.agents.filter(a => alive(a) && earnsWages(a.type)).map((a) => wageStatus(w, a));
  return {
...
export function choosePayJob(w: World, a: Resident) {
  if (!earnsWages(a.type)) return false;
