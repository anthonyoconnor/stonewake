...
export function tickFighter(w:World,a:Resident,dt:number,release:JobRelease,move:Move){
  ...
  const target=enemies.filter(e=>distance(e)<=combatDefaults.reach||fighter&&!a.recovering&&distance(e)<=combatDefaults.noticeRadius&&(!rally||Math.hypot(e.x-rally.x,e.z-rally.z)<=rally.radius)).sort((p,q)=>distance(p)-distance(q)||p.id-q.id)[0];
  if(target){
