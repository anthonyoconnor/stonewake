...
export const settings:Setting[]=[
 ...
 ...characterDefinitions.flatMap(c=>c.levels.flatMap(level=>[
  field(`dwarf.${c.id}.level.${level.level}.wage`,`${c.name} · level ${level.level} · wage per payday`,`${c.name} levels`,level,'wage',c.construct?0:1,c.construct?0:1000,1,'Future paydays only; already owed wages keep their original amount.'),
