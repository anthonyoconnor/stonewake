...
import { showcaseBuiltId, showcaseLevel } from './settlement-showcase.ts';
import { createBuiltShowcase } from './settlement-showcase-build.ts';

export interface LevelPreviewEntry {
  id: string; name: string; level: LevelDefinition;
  group: 'Campaign' | 'Standalone' | 'Showcases' | 'Before overhaul' | 'Authoring';
}
...
export const levelPreviewEntries: LevelPreviewEntry[] = [
  { id: showcaseBuiltId, name: 'Hearthside Halls · Built base', level: showcaseLevel, group: 'Showcases' },
...
export function createLevelPreviewWorld(id: string, current: World): World {
  const next = id === 'current' ? structuredClone(current)
    : id === showcaseBuiltId ? createBuiltShowcase()
