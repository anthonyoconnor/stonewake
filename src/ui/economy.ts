...
import { characterLevel, isConstruct } from '../content/characters';
...
export function updateEconomy(sidebar: Sidebar) {
  ...
  status.textContent = payroll.unpaid
    ...
    : w.agents.some(a=>!isConstruct(a.type)) ? `All wages paid. Next payday in ${Math.max(0, Math.ceil(payroll.nextAt - w.elapsed))} seconds. Dwarfs visit a Treasure Room or the Hearth treasury.` : 'No dwarfs to pay. Stonehands need no wages.';
}

export function residentPayText(w: World, a: Resident) {
  if(isConstruct(a.type))return 'No wages.';
