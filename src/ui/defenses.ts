...
export function showDefenses(s:Sidebar){
  s.panel.innerHTML=`<p class="eyebrow">DEFENSES</p><div id="defense-inspector"></div><details id="build-defenses" open><summary>Build defenses</summary><p class="muted">Manufacture in the Workshop, then place from stock on clear claimed floor. Doors fit one-square passages between walls.</p><div class="defense-catalog">${defenseDefinitions.map(d=>{const r=recipeById(d.id)!;return `<button data-defense="${d.id}" class="defense-choice">${actionIcon(d.id)}<span><strong>${d.name}${d.tier?` · ${d.tier}`:''}</strong><small>${r.cost} gold · ${r.seconds}s work${d.health?` · ${d.health} health`:` · ${d.damage} damage`}<br><span data-stock="${d.id}"></span></small></span></button>`;}).join('')}</div><label>Bolt facing<select id="defense-facing">${defenseDirections.map((d,i)=>`<option value="${i}" ${i===s.selection.rotation?'selected':''}>${d.name}</option>`).join('')}</select></label><p class="muted">R rotates a bolt before placement. The arrow shows its firing direction.</p><button id="inspect-defense" class="wide">Inspect placed defense</button><p id="defense-description" class="muted"></p></details><details><summary>Placed defenses</summary><div id="placed-defenses"></div></details>`;
...
export function updateDefenses(s:Sidebar){
  ...
  for(const def of defenseDefinitions){
    s.panel.querySelector(`[data-stock="${def.id}"]`)!.textContent=`${w.outputs[def.id]??0} in stock`;
    const button=s.panel.querySelector(`[data-defense="${def.id}"]`)!;button.classList.toggle('active',s.selection.tool===def.id);button.setAttribute('aria-pressed',String(s.selection.tool===def.id));
  }
  const selectedDef=defenseById(s.selection.tool);
  s.panel.querySelector('#defense-description')!.textContent=selectedDef?selectedDef.description+(selectedDef.cooldown?` Cooldown: ${selectedDef.cooldown}s.`:'')+(selectedDef.pinSeconds?` Pins for ${selectedDef.pinSeconds}s.`:''):'';
