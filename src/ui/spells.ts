...
export function showSpells(sidebar: Sidebar) {
  ...
  sidebar.panel.querySelectorAll<HTMLButtonElement>('[data-cast]').forEach(
    (b) =>
      (b.onclick = () => {
        if(b.dataset.cast===summonMinerSpell.id){
          sidebar.selection.setTool('dig');
          sidebar.root.querySelector('#feedback')!.textContent=castSpell(sidebar.view.world,summonMinerSpell.id);
          sidebar.update();return;
        }
...
export function updateSpells(sidebar: Sidebar) {
  ...
  if (researchCapacity) {
    ...
    const summonCast=sidebar.panel.querySelector<HTMLButtonElement>('[data-cast="summon-miner"]')!;
    summonCast.disabled=!quote.eligible;
    summonCast.textContent=`Cast · ${quote.price} gold`;
    if(summonSelected){
