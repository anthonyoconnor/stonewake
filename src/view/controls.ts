...
export class CameraControls {
  ...
  constructor(view:GameScene) {
    ...
    canvas.addEventListener('pointermove',e=>{
      ...
      this.rotate(-(e.clientX-this.drag.x)*.006);
  ...
  update(dt:number){
    ...
    if(this.pointer&&!this.drag){
      const {x,y}=this.pointer,edge=18;
    ...
    if(orbit&&horizontal)this.rotate(horizontal*dt*1.2);
    if(this.keys.has('KeyQ'))this.rotate(-dt*1.2);
    if(this.keys.has('KeyE'))this.rotate(dt*1.2);
  }
}
