...
export class StartingHearthView {
  ...
  update() {
    ...
    const crystal = crystalMesh(v, stone.x, 0.72, stone.z, 1.05, active ? '#8af1e3' : '#5991b2', root);
...
function startingOnwardPhase(v: GameScene, parent: TransformNode, active: boolean) {
  const root = new TransformNode('starting onward ' + active, v.scene),
    stone = { x: 0, z: 0 };
  root.parent = parent;
  const plinth = MeshBuilder.CreateCylinder(
    'onward stone plinth',
...
export function createStartingOnwardHearthModel(view: GameScene, parent?: TransformNode) {
  const root = new TransformNode('starting onward hearthstone', view.scene);
  if (parent) root.parent = parent;
  const dormant = startingOnwardPhase(view, root, false),
    active = startingOnwardPhase(view, root, true);
  active.setEnabled(false);
  return { root, dormant, active };
}
export function updateStartingOnwardHearthModel(
  model: ReturnType<typeof createStartingOnwardHearthModel>,
  pose: OnwardHearthPose,
) {
  const active = pose.ready || pose.progress > 0;
  model.dormant.setEnabled(!active);
  model.active.setEnabled(active);
}
