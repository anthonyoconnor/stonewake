...
export class SceneEffects {
  ...
  emit(x:number,y:number,z:number,kind:'dust'|'spark'|'steam',count:number){
    for(let i=0;i<count;i++){
      ...
      const spark=kind==='spark',steam=kind==='steam';p.duration=steam?1.5:spark?.45:.65;p.life=p.duration;p.size=steam?.11:spark?.035:.065;p.gravity=steam?0:3;
      p.mesh.material=this.view.material(`effect-${kind}`,steam?'#b8c2bb':spark?'#ffc276':'#947f60',false,spark?1:0);
      p.mesh.position.set(x,y,z);p.mesh.setEnabled(true);p.velocity.set((Math.random()-.5)*(steam?.15:1.3),steam?.3:.6+Math.random()*.8,(Math.random()-.5)*(steam?.15:1.3));
    }
  }
  update(){
    ...
    if(!this.reduced){
      for(const a of w.agents){const j=a.job;if(!j||a.path.length||!['mine','craft','claim'].includes(j.kind)){this.beats.delete(a.id);continue;}
        const beat=Math.floor(j.progress*2);if(this.beats.get(a.id)===beat)continue;this.beats.set(a.id,beat);
